...
public class Morph {
	...
	public Response morphify(String base64Codes ) {
		
		String[] imagesCodes = base64Codes.split(",");
		
		
		BufferedImage sImage = convertToImage(imagesCodes[1]);
		BufferedImage eImage = convertToImage(imagesCodes[3]);
		...
		int[][] imagesPixels = morph(sImagePixels, eImagePixels,6);
		
		String outputBase64Codes = "";
		...
		for( int i=0; i< imagesPixels.length; i++) {
			...
			outputBase64Codes += convertToBase64(image, "png" ) +",";
		}
		
		//return outputBase64Codes;
		return Response.ok()
        .entity(outputBase64Codes)
        .header("Access-Control-Allow-Origin", "*")
        .build();
	}
	
	public String convertToBase64(BufferedImage image, String type) {
